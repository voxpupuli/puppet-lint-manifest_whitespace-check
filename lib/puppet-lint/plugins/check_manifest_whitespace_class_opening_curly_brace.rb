...
PuppetLint.new_check(:manifest_whitespace_opening_curly_brace_before) do
  def check
    tokens.select { |token| token.type == :LBRACE }.each do |brace_token|
      ...
      next if %i[LPAREN LBRACK LBRACE COMMA].include?(prev_code_token.type)
      next unless tokens.index(prev_code_token) != tokens.index(brace_token) - 2 ||
                  !is_single_space(prev_token)

      notify(
        :error,
        message: 'there should be a single space before an opening curly brace',
